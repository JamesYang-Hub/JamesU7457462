...
```{r eval=FALSE}
# According to the correlation test, given the cor is-0.4719156，body mass and bill depth show a negative correlation, that is, as body mass increases bill depth decreases,given the pvalue is less than 0.05,which tell us that the experimental results are statistically significant.
```
...
```{r eval=FALSE}
# The plot shows that it appears that the bill depth increases with increasing body mass in different weight ranges, but the overall trend is for the bill depth to continue to decrease with increasing body mass.
```
...
```{r eval=FALSE}
# Yes, the trend line shows the negative correlation between bill depth and body mass,actually, I don't think regression is a good model to present the relationship between bill depth and body mass, the plot obviously shows that there may be a reversal correlation in different body mass regions.
```
...
```{r}
ggplot(penguins,aes(y=body_mass_g,x=species))+geom_point()+facet_wrap(~species,nrow = 1)+geom_boxplot()+geom_jitter()
```
...
```{r}
ggplot(penguins,aes(y=body_mass_g,x=species))+geom_point()+facet_wrap(~species,nrow = 1)+geom_violin()+geom_jitter()
```
...
```{r}
ggplot(penguins,aes(y=body_mass_g,x=species,col=species))+geom_point()+facet_wrap(~species,nrow = 1)+geom_violin()+geom_jitter()
```
...
```{r}
ggplot(data = penguins, mapping = aes(x = body_mass_g, y =bill_depth_mm ,col=species)) +
  geom_point()+geom_smooth(method='lm')+facet_wrap(~species,nrow=1)
```
...
```{r}
penguins %>% filter(species == "Adelie") %>% ggplot(mapping = aes(x = body_mass_g,y = bill_depth_mm,colour=island)) + geom_smooth(method = 'lm') +geom_point()
```
...
```{r}
Ade_number_male<-male_penguins<-penguins %>% filter(species == "Adelie")%>% filter(sex=='male')%>%count(sex)
Ade_number_female<-female_penguins<-penguins %>% filter(species == "Adelie")%>% filter(sex=='female')%>%count(sex)
Chi_number_male<-male_penguins<-penguins %>% filter(species == "Chinstrap")%>% filter(sex=='male')%>%count(sex)
Chi_number_female<-female_penguins<-penguins %>% filter(species == "Chinstrap")%>% filter(sex=='female')%>%count(sex)

Gen_number_male<-male_penguins<-penguins %>% filter(species == "Gentoo")%>% filter(sex=='male')%>%count(sex)
Gen_number_female<-female_penguins<-penguins %>% filter(species == "Gentoo")%>% filter(sex=='female')%>%count(sex)
Ade_number_male
Ade_number_female
Chi_number_male
Chi_number_female
...
#   male_penguins<-penguins %>% filter(!(is.na(sex))) %>%filter(species == "Adelie")%>% filter(sex=='male')
# female_penguins<-penguins%>% filter(!(is.na(sex))) %>% filter(species == "Adelie")%>% filter(sex=='female')
male_penguins<-penguins %>% filter(!(is.na(sex))) %>% filter(sex=='male')
female_penguins<-penguins%>% filter(!(is.na(sex))) %>% filter(sex=='female')
head(penguins)
```
...
```{r}
penguins%>%filter(!(is.na(sex))) %>% ggplot(aes(y=body_mass_g, x = sex, colour=sex))+facet_grid(~species) + geom_boxplot() +  geom_jitter(width = 0.5)
```
...
```{r}
penguins%>%filter(!(is.na(sex))) %>%ggplot( mapping = aes(x = body_mass_g, y =bill_depth_mm ,col=sex)) +
  geom_point()+geom_smooth(method='lm')
...
```{r eval=FALSE}
# I think sex has a big influence on the depth of the bill, as can be seen from the plot where it is clear that the depth of the bill is greater in males than in females at the same weight.
```
...
```{r eval=FALSE}
# (1)Solves a problem that would have required a lot of code to solve the plot creating
# (2)Multiple charts can be displayed in one chart, making it easier to compare data
# (3)A more intuitive way to determine the correlation between the data and plots by showing the corresponding pairwise correlations.
```
...
```{r message = FALSE, warning = FALSE}
penguins %>%filter(!(is.na(sex)))%>% select(bill_length_mm,bill_depth_mm,flipper_length_mm,body_mass_g,sex) %>% ggpairs(aes(colour = sex,alpha= .4))
...
penguins %>%
# delete the NA data in sex column
filter(!(is.na(sex))) %>% 
# add the year as a factor into the tibble
mutate(year = as.factor(year)) %>% 
# create the ggplot and aesthetic xlim and ylim and add species as color 
ggplot(mapping = aes(x = species,y = flipper_length_mm, colour=species)) + 
# add the boxplot 
geom_boxplot() + 
# separate the points in plot
geom_jitter(width = 0.2, height = 0, alpha = 0.4) + 
# use the facet_grid() to demonstrate the effect of sex and year on flipper_length_mm
facet_grid(sex ~ year) + 
...
```{r eval=FALSE}
# The most informative measyrement is flipper length, there is obvious difference among three species in theboxplot. The least informative measurement is the body mass, there is no difference in species except Gentoo.
# I think I would like to choose the bill depth and flipper length, from the result of filpper length, Gentoo could be identified and combined with the result of bill depth, which shows the difference between Adelie and Chinstrap.
```
...
```{r message = FALSE, warning = FALSE}
penguins %>% select(bill_depth_mm,flipper_length_mm,species) %>% ggpairs(aes(colour = species))
```
