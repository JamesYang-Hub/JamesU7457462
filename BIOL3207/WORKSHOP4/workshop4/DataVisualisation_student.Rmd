...
```{r}
library(tidyverse)
cancer_df <- read_csv('./data/Ruiz_Aravena_et_al_raw_data.csv')
...
If you look at the help file for your geom (or the cheat sheet), you can see what aesthetics you can modify.
...
```{r}
ggplot(data = cancer_df, mapping = aes(x = log_T_vol, y = BCestimate,shape = Site,col=Sex)) +
  geom_point()
```
...
```{r}
ggplot(cancer_df, aes(x=Sex_f, y = BCestimate,col=Sex_f))+
  geom_violin()+
  geom_jitter(width=0.2, height=0)
```
...
```{r}
ggplot(cancer_df, aes(x = log_T_vol, fill = Sex_f))+xlab('Log_T_vol')+
  geom_density(alpha= 0.5)+ facet_wrap(~Site)+scale_fill_brewer(palette = "PiYG")
ggplot(cancer_df, aes(x = log_T_vol, fill = Sex_f))+
  geom_density(alpha= 0.5)+ facet_wrap(~Site)+theme_bw()
```
...
```{r}
ggplot(data = cancer_df, mapping = aes(x = log_T_vol, y = BCestimate,colour = Sex_f, shape = Site)) +
  geom_point()+geom_smooth()+facet_wrap(~Sex_f,ncol = 1)
ggplot(data = cancer_df, mapping = aes(x = cut(log_T_vol,c(-Inf,1,2,3,4,Inf)), y = BCestimate,colour = Sex_f, shape = Site)) +
  geom_point()+geom_smooth()+facet_wrap(~Sex_f,ncol = 1)+geom_jitter(width=0.2, height=0)#+cut(cancer_df$log_T_vol,c(-Inf,1,2,3,4,Inf))
```
