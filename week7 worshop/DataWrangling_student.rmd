...
```{r echo=FALSE}
calls_file <- "data/ecoli_variants_calls.xlsx"
...
```
```{r}
SRR2584866_calls<-add_column(SRR2584866_calls,sample_id = sample_ids[2])
SRR2589044_calls<-add_column(SRR2589044_calls,sample_id = sample_ids[3])
...
```{r}
calls <- bind_rows(SRR2584863_calls,SRR2584866_calls,SRR2589044_calls)
...
transitions <- calls %>% filter((REF == "A" & ALT == "G") | REF == "G" & ALT == "A")
...
```{r}
calls <- calls %>% mutate(PROB = 1 - (10^-(QUAL/10)))
```
...
```{r}
calls <- calls %>% mutate(INDEL = (nchar(ALT) != nchar(REF)))
calls
```
...
```{r}
calls%>%ggplot(aes(y=QUAL,x=POS,col=INDEL))+geom_jitter(width = 1)+geom_smooth()
```
...
```{r}
calls %>% group_by(INDEL) %>% summarise(n = n())
```
...
```{r}
calls%>% group_by(sample_id,INDEL)%>% select(sample_id,INDEL)%>% summarise(n = n())
```
...
```{r}
details_file <- "data/ecoli_variants_details.xlsx"
...
SRR2584863_details <- add_column(SRR2584863_details,sample_id = sample_ids[1])
SRR2584866_details<- add_column(SRR2584866_details,sample_id = sample_ids[2])
SRR2589044_details <- add_column(SRR2589044_details,sample_id = sample_ids[3])
details <- bind_rows(SRR2584863_details,SRR2584866_details,SRR2589044_details)
details
```
...
```{r}
variants <- inner_join(calls,details,copy=F,by=c("POS","sample_id","CHROM"))
variants
```
...
```{r}
variants %>%
  group_by(sample_id, CHROM, INDEL.x) %>%
