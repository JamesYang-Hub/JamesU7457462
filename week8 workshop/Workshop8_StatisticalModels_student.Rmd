...
(difference_from_mean could be considered a reasonable measure of "wrongness")#? 770=39*19
...
#? F-value is the Mean Sq for sex(489)/the Residual Mean Sq(7.4)
...
```{r}
df_numeric <- df %>% mutate(sex=ifelse(sex=="FEMALE",0,1))
df_numeric
...
```{r}
lm_two_factor <- lm(mass ~ sex+trt+sex:trt,data=two_factor)#? sex:trt means
...
the obesity drug has higher effect on female, it is obvious that the plot shows the difference between two intercepts of the female is bigger than male's.
...
```{r}
ggplot(two_factor_interaction,aes(x = factor(2), y = mass)) + geom_violin() + geom_point(size=2,alpha=0.5) + stat_summary(fun = "mean",
               geom = "crossbar", 
...
```{r}
age_sex_df_test <- tibble(mouse_id = 1:n_mice, age = sample(20,40,replace=TRUE), sex = c(rep("FEMALE",n_mice/2),rep("MALE",n_mice/2))) %>% mutate(mass = ifelse(sex == "FEMALE",intercept+age*female_slope,intercept+age*male_slope)) %>% mutate(mass = mass + rnorm(n_mice,0,noise))
...
model_mixed_lme <- lmer(change_in_mass ~ age*sex*trt + (1|cage), data = cages_paired_age_sex_trt_df)#？(1|cage) means cage doesn't affect the coefficient (slope) of change_in_mass is a random variance
