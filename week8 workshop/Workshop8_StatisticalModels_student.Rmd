...
```{r}
age_sex_df_test <- tibble(mouse_id = 1:n_mice, age = <<TRY SOMETHING HERE>>, sex = c(rep("FEMALE",n_mice/2),rep("MALE",n_mice/2))) %>% mutate(mass = ifelse(sex == "FEMALE",intercept+age*female_slope,intercept+age*male_slope)) %>% mutate(mass = mass + rnorm(n_mice,0,noise))
