...
assign <- assign %>% group_by(Sample_id,Year,Species)# group by Species,Sample_id and Year
...
assign <- assign %>% mutate(Species = stringr::str_replace(Species,"A Mendacium","A mendacium"),Year = stringr::str_replace(Year,"2217","2017"))
...
### Create the ggplot of hegiht and weight between ***A fictus***  and ***A mendacium***
```{r}
ggplot(assign,aes(x=Weight))+geom_density(aes(col= Species))
ggplot(assign,aes(x=Height))+geom_density(aes(col= Species))
