...
[My GitHub Repository](https://github.com/JamesYang-Hub/JamesU7457462)
...
## Cloning provides a copy of all the files from the repository and then it places them on your computer in an area. because it allows the team to collaborate, inevitably, there will be conflict.
...
# The code will not be shown in the final document
```
...
## The red highlights is the deletion, the green highlights are the changes and additions.
```
...
```{r, prettyfig, fig.align='center', fig.cap="fig1"}
library(png)
...
Note the code chunk used to make the figure. It has a `fig.cap` argument. That means Rmarkdown knows it's a figure and it will allow you to create a figure reference call. In other words, we can refer to our Figure \@ref(fig:prettyfig) #问题#by referring to the label for the chunk. This will automatically make a legend for us too (assuming you add one in). The same concept applied to Tables but the legend goes above these.
...
```{r collabFig,fig.cap="fig2", echo=TRUE, eval=TRUE}
...
# mean_difference[i] <-  abs((drop_data %>%
#                               filter(species==data_species[i]& treatment== 'control') %>%
#                               summarise(mean = mean(activity)))-(drop_data %>%
#         filter(species==data_species[i]& treatment== 'CO2') %>%
#  summarise(mean = mean(activity))))
#  pval[i] <- t.test(fit_control[3],fit_CO2[3])$p.value
# conf[i]<- t.test(fit_control[3],fit_CO2[3])$conf.int
# sta[i] <- t.test(fit_control[3],fit_CO2[3])$statistic
# }
# pval
# conf
# sta
# mean_difference <- unlist(mean_difference)
# mean_difference
# tibble(data_species,pval,conf,mean_difference,sta)
...
Mean activity for `r "humbug"` in the control group was `r drop_data %>% filter(species=='humbug'& treatment== 'control') %>% summarise(mean = mean(activity))` (s / min) compared to the OA treatment group, which was `r  drop_data %>% filter(species=='humbug'& treatment== 'CO2') %>% summarise(mean = mean(activity))` (s / min). The difference between control and OA treatment means was `r "(drop_data %>% filter(species=='humbug'& treatment== 'control') %>% summarise(mean = mean(activity)))-(drop_data %>% filter(species=='humbug'& treatment== 'CO2') %>% summarise(mean = mean(activity)))` (s / min) (95% CI: `r table_activity %>% filter(species=='humbug') %>% select(conf.low)` to `r table_activity %>% filter(species=='humbug') %>% select(conf.high)`).
...
`r "The NULL hypothesis is that there is no difference between two treatment about humbug specie  , the evidence of rejection is that the p-value is " table_activity[which(table_activity$species=='humbug'),] %>% select(p.value) `
...
permutation_pvalue #0.54
# quite different with previous result
```
