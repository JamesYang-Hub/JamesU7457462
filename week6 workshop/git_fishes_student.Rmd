...
# drop_data <- data %>% select(-loc,-comment)
drop_data
...
```{r}
file.create("D:/S2 2022/BIOL3207/week6/cloning repo/JamesU7457462/.gitignore.txt")
...
ggplot(data , aes(x =species, y =activity,col=treatment)) +geom_boxplot()+ ggtitle("difference between control and acidification treatments") + theme(plot.title = element_text(hjust = 0.5))+annotation_raster(acantho_logo,.5,1.5,50,60)+annotation_raster(ambon_logo,1.5,2.5,50,60)+annotation_raster(chromis_logo,2.5,3.5,50,60)+annotation_raster(humbug_logo,3.5,4.5,50,60)+annotation_raster(lemon_logo,4.5,5.5,50,60)+annotation_raster(whitedams_logo,5.5,6.5,50,60)
