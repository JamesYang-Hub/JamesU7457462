...
```{r sampldistZr}
#?
ggplot2::ggplot(tibble(Zr), aes(x = Zr)) + geom_histogram(binwidth = 0.02) + geom_vline(xintercept =  0.5*log((1+cor)/(1-cor)), col = "red") + geom_vline(xintercept = mean(Zr), col = "blue", linetype = 2)
...
std_taxa <- zr_data %>%
    group_by(taxa) %>%
    summarise(stdy = length(unique(study_ID)), effects = n())
std_taxa %>%
    ggplot(aes(x = taxa, y = stdy)) + geom_point(size = 6, aes(colour = effects)) +
